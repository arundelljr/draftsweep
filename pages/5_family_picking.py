...
if control_panel['Picking'][0] == 1:
    ...
    if len(set(top_picks)) != 15:
        ...
        if st.button("Save Picks"):
            ...
            google_sheets_df = conn.read(worksheet="family_final_picks")
            ...
            conn.update(data=google_sheets_df, worksheet="family_final_picks") # Save changes
...
# Allow drafting through control panel
    ...
    if st.button("Complete Draft"):
        ...
        final_picks = conn.read(worksheet="family_final_picks")
        df = pd.DataFrame(final_picks)
        complete_draft(df, "family_final_draft", "family_the_chosen_ones")
